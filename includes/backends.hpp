...
struct BackendCreationError : public IError {
  ...
  CREATE_BACK_CREATION_ERROR_CONSTR(vulkan_failed_to_load_instance,
                                    std::error_code);
  CREATE_BACK_CREATION_ERROR_CONSTR(vulkan_failed_to_select_device,
                                    std::error_code);
  CREATE_BACK_CREATION_ERROR_CONSTR(vulkan_failed_to_build_device,
                                    std::error_code);
  ...
  auto to_string() const -> std::string override final {
    ...
    case ErrorType::vulkan_failed_to_load_instance:
      return fmt::format("Vulkan failed to load instance, error {} -> {}",
                         std::get<0>(error).value(),
                         std::get<0>(error).message());

    case ErrorType::vulkan_failed_to_select_device:
      return fmt::format("Vulkan failed to select device, error {} -> {}",
                         std::get<0>(error).value(),
                         std::get<0>(error).message());

    case ErrorType::vulkan_failed_to_build_device:
      return fmt::format("Vulkan failed to build device, error {} -> {}",
                         std::get<0>(error).value(),
                         std::get<0>(error).message());
